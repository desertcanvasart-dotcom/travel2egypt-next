/** Publish the owner-approved Kom Ombo parity draft (wp-page-78376). */
...
async function main() {
  ...
  const cnt = (d) => ['en','es','ja'].map(l=>`${l}=${d.body.find(b=>b._key===l).value.length}`).join(' ');
  console.log(`${APPLY?'APPLY':'DRY'}: draft ${cnt(draft)} | published(pre) ${cnt(pub)}`);
  if (!APPLY) return;
  mkdirSync('backups', { recursive: true });
  writeFileSync('backups/komombo-parity-publish-rollback-2026-07-10.json', JSON.stringify(pub, null, 2));
