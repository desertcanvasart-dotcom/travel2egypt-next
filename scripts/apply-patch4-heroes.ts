/**
 * Apply the patch4 hero images to their matching docs in PRODUCTION.
 *
 *   Source: Desktop/.../img-depo/raw copy/4/patch1/<slug>.jpg
 *   Target: the `tour` OR `hotel` doc whose slug == <filename minus .jpg>.
 *
 * Same mechanics as apply-patch3-heroes.ts: resolve by literal slug (no assumed
 ...
 * Files that don't resolve to a doc are SKIPPED and reported (e.g. an
 * un-renamed stock photo) — never forced onto a page.
 *
 *   Dry run (default): npx tsx scripts/apply-patch4-heroes.ts
 *   Apply:             npx tsx scripts/apply-patch4-heroes.ts --apply
 */
...
const DIR = '/Users/islamhussein/Desktop/Projects/T2E/img-depo/raw copy/4/patch1';
...
async function main() {
  ...
  console.log(`\n${ok}/${files.length} mapped (${rollback.filter((r) => r.docType === 'tour').length} tours, ${rollback.filter((r) => r.docType === 'hotel').length} hotels). skipped: ${skipped.length ? skipped.join(', ') : 'none'}`);

  if (APPLY) {
    const logPath = path.join(process.cwd(), 'backups', 'patch4-heroes-rollback.json');
