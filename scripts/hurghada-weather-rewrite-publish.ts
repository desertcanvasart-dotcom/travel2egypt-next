...
async function main() {
  ...
  const path = 'backups/hurghada-weather-pre-rewrite-2026-07-11.json';
  ...
  const now: any = await c.getDocument(ID);
  const dr: any = await c.getDocument('drafts.' + ID);
  console.log('verify title[en]=', now.title.find((t:any)=>t._key==='en').value);
  console.log('verify body[en] blocks=', (now.body.find((b:any)=>b._key==='en').value||[]).length, '| draft gone=', !dr);
  console.log('verify hero=', now.heroImage?.asset?._ref, '| seo locales=', (now.seo?.metaDescription||[]).map((x:any)=>x._key).join(','));
}
