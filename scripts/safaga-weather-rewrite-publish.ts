...
async function main() {
  ...
  const heroPrePublish = JSON.stringify(pub.heroImage);
  const path = 'backups/safaga-weather-pre-rewrite-2026-07-11.json';
  ...
  console.log('verify seo=', (now.seo?.metaDescription||[]).map((x:any)=>x._key).join(','));
  console.log('HERO byte-identical post-publish:', JSON.stringify(now.heroImage)===heroPrePublish, '(asset', now.heroImage?.asset?._ref+')');
  // also vs the pre-build snapshot captured before staging
  if (fs.existsSync('/tmp/safaga-hero-prebuild.json')) {
    console.log('HERO byte-identical vs pre-build snapshot :', JSON.stringify(now.heroImage)===fs.readFileSync('/tmp/safaga-hero-prebuild.json','utf8'));
  }
}
