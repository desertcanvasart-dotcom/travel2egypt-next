...
create table concierge.briefs (
  ...
  -- brief_revision int added in 0005 (Session 9)
);
...
Later migrations (as actually numbered in the repo): `0002` (conversation tour-context, Session 3), `0003` (escape-hatch columns, Session 5), `0004` (rate-limit actions, Session 7), `0005` (`brief_revision`, Session 9), `0006` (`anonymized_at`, Session 10). *(The original brief's "0002 escape-hatch / 0003 brief_revision" forward-references drifted as 0002 was taken for tour-context and 0004 for rate limiting; this line is the reconciled truth.)*
...
7. **Revisions (migration `0005`):** a material update appends a new `briefs` row with incremented `brief_revision` and `is_update: true`; the team dedups by `conversation_id`. *(Whether Autoura updates-by-`conversation_id` or creates-new-each-time is resolved with the Autoura team and reflected in team operating principles; non-blocking.)*

**Migration `0005`:** `alter table concierge.briefs add column brief_revision int default 1;` *(Applied manually by Islam in the Supabase dashboard. Timing: must be applied **before any `briefs` row carries a revision** — i.e. before live Phase 3 delivery — not earlier. Renumbered from the original brief's "0003"; see the reconciled migrations line in the Database section.)*

**S4-followup deferral (S9 scope boundary).** S9 builds and unit-tests correct next-`brief_revision` computation (from existing `briefs` rows for the conversation) so a second row is stamped correctly *when one is ever inserted*. It does **not** build the S4 re-extraction / "material update" trigger — `/api/brief` early-returns on an already-completed conversation, so in practice only revision 1 is produced today. Enabling the update trigger (detect a material change → re-extract → insert revision 2) is an explicit **S4 follow-up**, deferred out of S9 by decision; the revision path ships built-but-dormant.
...
8. **Migration `0006` — `anonymized_at timestamptz` — REQUIRED (not optional), deferred from S8.** *(Renumbered 0005 → 0006: S9 took 0005 for `brief_revision`. Was flagged at S9 Phase 0 to avoid an S10-kickoff collision.)* S8's delete-anonymise marks `archived=true` but carries **no timestamp**. Add `anonymized_at` (table TBD — likely `sessions`, since anonymise is session-scoped; surface per-conversation via the join) and set it in the S8 `/api/data-request` delete path once the column exists. Two reasons it's required: (a) **GDPR accountability** — we must be able to evidence *when* an erasure request was honoured; (b) **this panel needs it** — reviewers seeing gutted conversations (`'[deleted]'` content) must distinguish *"visitor exercised deletion on &lt;date&gt;"* from data corruption. S10 already opens migration-friendly, so it folds in cleanly.

**Decisions to flag.** Server-mediated admin + two-factor check (session AND whitelist), RLS deny-all, no permissive policies (locked, revises S2); `/admin` middleware exclusion + robots disallow (locked); Supabase Auth mail via Resend, separate from-address (locked); Railway cron digest at 08:00 Cairo, `CRON_SECRET` long-lived (locked); ILIKE search (locked); reviewer note categories as listed (locked); full-access whitelist, revoke = remove + redeploy, v2 → DB table (locked); `anonymized_at` migration 0006 (locked REQUIRED, deferred from S8; renumbered from 0005 after S9 took 0005 for `brief_revision`).
...
**Concierge link-map curation in `production` (Link Map session — named gate):** the runtime reads `conciergeLinkMap` from the active dataset (prod = `production`); the build verified it on `migration-staging`, so concierge deep-linking is **dormant in production until the singleton is curated there**. **Script:** [`scripts/seed-concierge-link-map.ts`](../../scripts/seed-concierge-link-map.ts) (`npm run seed:link-map -- --dataset=production --dry-run` to verify; drop `--dry-run` to write). It is dataset-parameterized and, per entry, **re-verifies the target exists + is robots-indexable in the *target* dataset before adding it**. **Sequencing finding (dry-run, this session):** the current `production` dataset is still the **legacy/pre-migration set** (5 `wikiMonument` vs 134 in `migration-staging`; a *different `_id` scheme* — `wiki-monument-edfu` vs `wp-page-*`), so all 6 seed targets report **not found** there today. This is **not slug drift** — it means the link-map seed is gated on the **content cutover** (the migration content reaching `production`). At S12: run the **`--dry-run` against the live `production` first**; if targets still aren't found by `_id`, update the seed ids/keys to the post-cutover values, then write — then curate further in Studio on prod. Content-readiness, not code: if missed, deep-linking is simply absent and the chat is unaffected (the loader fails closed to `[]`, live-verified). Sits on this gate list alongside the Supabase Free→Pro upgrade and org-ownership move.
...
- **`comfort_level` vocabulary + a receiver-side mapping gap (Session 9 — REPORT to the Autoura owner, do NOT patch on the wire):** v4.1's `briefExtraction.ts` emits `comfort_level` from the controlled set **`budget | mid-range | boutique | international-5-star | luxury`** (or a short free phrase the traveler used, or `null`). The Autoura receiver's provisional `COMFORT_LEVEL_TO_TIER` map keys `'international 5-star'` (a **space**) → `'deluxe'`, but v4.1 sends `'international-5-star'` (a **hyphen**), which misses the map *and* every keyword-fallback branch (`lux`/`deluxe`/`comfort`/`budget`) → it silently lands as **`'standard'`**. Net effect: a traveler signalling near-the-top comfort is filed as a mid-tier `preferred_tier` on Autoura — **a misclassification of our highest-value leads.** The sender sends `comfort_level` **verbatim** (correct — pre-spacing it on our side would hide the bug); the fix belongs on the Autoura side (`lib/concierge-brief-schema.ts`: add `'international-5-star'` and the bare `'boutique'`/`'mid-range'` hyphenated forms, and decide the intended tier for `international-5-star` — likely `deluxe` or `luxury`). **Out of scope for S9 to fix; in scope to document — this is the written report for the Autoura owner.** **LIVE-CONFIRMED (S9 Phase 3 dry-run against production, 2026-06-12):** a real dry-run send carrying `comfort_level: "international-5-star"` came back from the live receiver with `mapping_preview.preferences.preferred_tier: "standard"` — the misclassification is reproduced against the real Autoura code, not merely predicted. Recommended Autoura-side fix before launch: map `international-5-star` → `luxury` (or `deluxe`), and add the bare hyphenated `boutique`/`mid-range` forms, in `lib/concierge-brief-schema.ts`.

- **Node-only delivery chain pulled into the Edge instrumentation bundle (Session 9 — SAME CLASS as the S4 Edge-crypto finding):** `instrumentation.ts` is compiled for **every** runtime the app uses, and this app has `middleware.ts` → an Edge instrumentation bundle exists. The startup reconciler's chain (`reconcile → deliver → sign`) uses `node:crypto` + the service-role client, which the Edge bundle cannot resolve. Two symptoms in sequence: bare `import 'crypto'` → *"Can't resolve 'crypto'"*; `import 'node:crypto'` → *"UnhandledSchemeError: node:crypto"* in Edge. The fix is **two parts**: (1) use `node:crypto`, and (2) gate the dynamic import behind the **positive** `if (process.env.NEXT_RUNTIME === 'nodejs') { await import(...) }` form — Next's webpack plugin statically recognizes that exact condition and strips the branch from the Edge build; an early-return *negation* (`if (!== 'nodejs') return`) is **not** eliminated and drags the chain into Edge. Like the S4 Edge-crypto and S7 token-cap findings, **only the production build surfaced this** — `tsc` and the tsx unit tests were blind (they never compile for Edge). Lesson: anything reachable from `instrumentation.ts` that is Node-only must sit behind the positive-runtime-guarded dynamic import, and a green `tsc`/unit run does not prove the Edge bundle compiles — run `next build`.
- **tsx live-verification scripts need a `WebSocket` polyfill for `@supabase/supabase-js` (Session 9):** the Supabase client constructs a `RealtimeClient` eagerly in its constructor, which needs a global `WebSocket`. The Next.js runtime provides one; **plain Node 20 under `tsx` does not** (global `WebSocket` is stable only from Node 22), so any verification script that builds `conciergeDb()` throws *"Node.js 20 detected without native WebSocket support."* We never use realtime (PostgREST queries only), so the one-line fix is `if (!globalThis.WebSocket) globalThis.WebSocket = require('ws')` (the `ws` package is already a transitive dep) **before** the first client is built. Operational note for any future tsx script that touches the concierge DB directly — the app routes are unaffected.
- **Autoura webhook proven against PRODUCTION, not staging (Session 9 Phase 3, 2026-06-12):** there is **no Autoura staging environment** (a months-old "staging complete" note was wrong — verified in Railway). Live verification ran against **production** `www.autoura.net` deliberately, because at the time it had **zero live leads**, under a hybrid plan: dry-run sends (`X-Autoura-Dry-Run: true`, write nothing) for the keystone signature + mapping, and a **minimal real-send footprint (2 brief records + 1 deduped client)** for happy-path receipt / idempotency / reconciliation, with **mandatory cleanup the same sitting**. Boundary that held: the executor does **not** hold Autoura's service-role key and issues **no** DELETEs against the Autoura CRM — Autoura-side cleanup + confirm-empty is **Islam's**, gated before any merge. The keystone (`signature.valid: true` from the real verifier over the exact serialized bytes) is the end-to-end proof the unit vector cannot give. **Launch-ops note:** the receiver supports **dual-secret rotation** (`CONCIERGE_WEBHOOK_SECRET` + `_PREVIOUS`, both accepted) → secret rotation is zero-downtime via the `_PREVIOUS` overlap.
