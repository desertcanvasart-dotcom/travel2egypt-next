/**
 * Shared tour detail renderer.
 *
 * Used by both:
 *   - src/app/(site)/[locale]/[...rest]/page.tsx (root catch-all — canonical URL /<slug>)
 *   - src/app/(site)/[locale]/tours/[slug]/page.tsx (legacy explicit route, redirects to root)
 *
 * Renders day tours AND multi-day packages. Discriminator-aware sidebar
 * shows duration/mode/cities for day tours; duration/mode/theme/originRegion
 * for packages. ItineraryDays + Gallery + RelatedTours + GuideRefs +
 * TravelerStories all rendered where present.
 */
...
export async function TourPageView({ tour, locale, slug, siteSettings }: TourPageViewProps) {
  ...
  const whatsappNumber = siteSettings?.contact?.whatsapp;
  ...
  const modeLabel =
    tour.tourMode === 'private' ? t('modePrivate') :
    tour.tourMode === 'group' ? t('modeGroup') : null;
  ...
  const breadcrumbSchema = buildBreadcrumbList(
    [
      { name: 'Home', path: '/' },
      { name: tour.type === 'package' ? 'Packages' : 'Day Tours', path: tour.type === 'package' ? '/egypt-travel-packages' : '/private-day-tours' },
          ...
          <div>
            ...
            {tour.highlights && tour.highlights.length > 0 && (
              <section className="mt-16 border-t border-line pt-12">
                <h2 className="mb-6 font-serif text-3xl font-medium text-ink">{t('highlightsLabel')}</h2>
                <ul className="space-y-3 text-ink-soft">
                  {tour.highlights.map((h: string, i: number) => (
                    <li key={i} className="flex gap-3 leading-relaxed">
                      <span aria-hidden className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-orange-deep" />
                      <span>{h}</span>
                    </li>
                  ))}
                </ul>
            ...
            */}
                ...
                {Array.isArray(tour.inclusions) && tour.inclusions.length > 0 && (
                  <div>
                    ...
                    <div className="prose-editorial max-w-none text-ink-soft">
                      <Body value={tour.inclusions} locale={locale} />
                    </div>
                  </div>
                )}
                {Array.isArray(tour.exclusions) && tour.exclusions.length > 0 && (
                  <div>
                    ...
                    <div className="prose-editorial max-w-none text-ink-soft">
                      <Body value={tour.exclusions} locale={locale} />
                    </div>
                  </div>
                )}
              </section>
            ...
            {tour.relatedTours && tour.relatedTours.length > 0 && (
              <section className="mt-20 border-t border-line pt-16">
                <h2 className="mb-8 font-serif text-3xl font-medium text-ink">{t('relatedToursLabel')}</h2>
            ...
            {guideRefs.length > 0 && (
              <section className="mt-20 border-t border-line pt-16">
                <h2 className="mb-8 font-serif text-3xl font-medium text-ink">{t('relatedGuidesLabel')}</h2>
            ...
            {tour.relatedTravelerStories && tour.relatedTravelerStories.length > 0 && (
              <section className="mt-20 border-t border-line pt-16">
                <h2 className="mb-8 font-serif text-3xl font-medium text-ink">{t('travelerStoriesLabel')}</h2>
                <div className="space-y-8">
                  {tour.relatedTravelerStories.map(
                    (story: { _id: string; title?: string; excerpt?: string; authorName?: string; authorOrigin?: string }) => (
                      <figure key={story._id} className="border-l-2 border-orange pl-6">
                        {story.excerpt && (
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-lg border border-line bg-cream-warm p-6">
              <dl className="space-y-4 text-sm">
                {(tour.durationLabel || durationHoursLabel) && (
                  <div>
                    <dt className="mb-1 font-sans text-xs font-semibold uppercase tracking-wider text-ink-muted">{t('durationLabel')}</dt>
                    <dd className="text-ink-soft">{tour.durationLabel || durationHoursLabel}</dd>
                  </div>
                )}
                {modeLabel && (
                  <div>
                    <dt className="mb-1 font-sans text-xs font-semibold uppercase tracking-wider text-ink-muted">{t('modeLabel')}</dt>
                    <dd className="text-ink-soft">{modeLabel}</dd>
                  </div>
                )}
                {cityList.length > 0 && (
                  <div>
                    <dt className="mb-1 font-sans text-xs font-semibold uppercase tracking-wider text-ink-muted">{t('citiesLabel')}</dt>
                    <dd className="text-ink-soft">{cityList.join(', ')}</dd>
                  </div>
                )}
                {tour.priceIndication && (
                  <div className="border-t border-line pt-4">
                    <dt className="mb-1 font-sans text-xs font-semibold uppercase tracking-wider text-ink-muted">{t('priceIndicationLabel')}</dt>
                    <dd className="font-serif text-base italic leading-relaxed text-ink">{tour.priceIndication}</dd>
                    <p className="mt-3 text-xs leading-relaxed text-ink-muted">{t('priceCaveat')}</p>
                  </div>
                )}
              </dl>
            </div>
