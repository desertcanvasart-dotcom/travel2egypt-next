...
/**
 * Locale resolver for tours (both `dayTour` and `package` subtypes).
 *
 * Mirrors /api/locale-resolve/travelTip — field-level i18n shape (single doc,
 * locale-keyed slug array). Returns the target-locale slug for the matching
 * tour, or null if no translation exists in that locale.
 *
 * Consumed by LocaleSwitcher's resolveLocalizedPathname() on language pick
 * from a /tours/[slug] or /packages/[slug] page. The caller is responsible
 * for re-attaching the correct base path (`/tours/` vs `/packages/`) — the
 * resolver returns the slug only.
 */

const lookupQuery = `*[_type == "tour" && (
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('[locale-resolve] tour fetch failed:', err);
