 ...
 * matching wikiMonument, or null if no translation exists in that locale.
 *
 * Consumed by LocaleSwitcher's resolveLocalizedPathname() on language
 * pick from a /wiki/monuments/[slug] page.
 */

const lookupQuery = `*[_type == "wikiMonument" && !(_id in path("drafts.**")) && (
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('[locale-resolve] wikiMonument fetch failed:', err);
